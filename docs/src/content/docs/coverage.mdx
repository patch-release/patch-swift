...
description: What an OTA patch can and cannot change in a SwiftUI app, with coverage measured on a real validation corpus and the one permanent binary-symbol wall.
sidebar:
  order: 4
sources:
...
An OTA patch can change SwiftUI structure, state and interaction, plus Swift
logic and `async`/`await`, in the app your users already have installed. It
cannot introduce a native symbol, framework or entitlement that is not already
compiled and signed into that binary. This page is the map of that boundary,
with per-app coverage measured on a real validation corpus.
...
Patch routes `View.body` getters through generated thunks and data-drives the renderer. It has no mechanism to rewrite arbitrary compiled native functions. So an OTA patch can't change the *body* of:
...
- [How it works](/how-it-works/) — the partitioning and packaging pipeline
- [FAQ](/faq/) — what a patch can and cannot change, in short form
- [Glossary](/glossary/) — mixed view, demote-to-native, binary-symbol wall
- [Hotfix a Swift app over the air](https://patchrelease.com/blog/hotfix-swift-app-over-the-air)
