---
title: Plans & billing
description: Hobby is free and serves up to 100 devices; Startup and Enterprise add team members, staged rollouts, channels beyond production, and a larger device fleet.
sidebar:
  order: 13
...
Every Patch workspace carries a billing **plan** — `hobby`, `pro` or `team` —
that gates a small set of team-scale features. Everything core to shipping OTA
updates is on the free tier: unlimited apps, unlimited 100% production
releases, instant rollback, the CLI, CI/CD, and the ship-safety checks. What
the paid plans add is collaboration, staged rollouts, channels beyond
`production`, a larger device fleet, and the audit log.
