...
description: Patch sends signed POSTs for release.pushed, rollout.changed, rollback and error_spike, so Slack or on-call hears about a release without polling a dashboard.
sidebar:
  order: 16
...
A Patch webhook is an HTTPS endpoint you register to receive workspace events in
real time — post a release to Slack, page on-call when an error rate spikes, or
kick off a downstream job. Patch sends a signed `POST` to every registered
endpoint subscribed to the event. They're managed by a workspace **owner** or **admin**, in the console under **Settings → Webhooks** or over the API.
