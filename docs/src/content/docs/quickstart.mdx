...
:::caution[Your app key ships inside your app]
`app_key` is compiled into your binary, so it can be recovered from a published
build. It is also accepted as a publish credential today, which means anyone who
extracts it can push a module to this app. For CI, set a separate
`PATCH_API_KEY` rather than relying on the baked key. See
