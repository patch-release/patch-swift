...
public enum N {
    ...
    /// A CALLBACK SLOT (see `NodeKind.callbackSlot`): a child-view call with a `() -> Void`
    /// closure arg whose body is OTA-dispatchable (state mutations over marshalled inputs). The
    /// child view itself is a native opaque slot (the thunk supplies the real `AnyView`); only
    /// the closure BODY lives in WASM (OTA-patchable). The `label` is for diagnostics.
...
extension ViewNode {
    ...
    /// A NATIVE EFFECT-MODIFIER SLOT (see `Modifier.nativeEffectSlot`): an undispatchable
    /// EFFECT modifier (`.task`/`.onAppear`/`.onTapGesture(perform:)`/a gesture/etc.) kept
    /// NATIVE while its receiver subtree lowers to WASM. The renderer re-applies the thunk's
    /// `__patchEffectSlots()[id]` `(AnyView) -> AnyView` closure over `self` to this node's
    /// rendered content. The emitter chains `<base>.nativeEffectSlot(id:)` on the lowered
    /// receiver — without this helper the generated guest module fails to compile (`value of
    /// type 'ViewNode' has no member 'nativeEffectSlot'`), isolating every effect-slot view.
    public func nativeEffectSlot(id: String) -> ViewNode { with(.nativeEffectSlot(id)) }
