...
/// The non-finite wire sentinels. MUST match `PatchViewIR.JSONNonFinite` in the SDK
/// and the `ABI.makeEncoder()` / `makeDecoder()` strategies — the host decodes these
/// strings back to `.infinity` / `NaN`.
...
struct JSONOut {
    ...
    mutating func number(_ d: Double) {
        // JSON has no representation for NaN / ±Infinity, and `String(Double.nan)` /
        // `String(Double.infinity)` produce the bare tokens `nan` / `inf` — NOT valid
        // JSON. A non-finite layout value would emit a blob the host `JSONDecoder`
        // can't parse, so the WHOLE lowered body fails to render.
        //
        // Emit the non-finite SENTINEL STRINGS the host decodes via
        // `nonConformingFloatDecodingStrategy.convertFromString` (see `ABI.swift`).
        // This is the T2 (Foundation) guest's encoding too, which is the point:
        // `ABI.makeEncoder()` is documented as carrying non-finite doubles "as the
        // same string tokens the embedded emitter uses", so both guest tiers must
        // produce byte-identical wire output for the same view.
        //
        // This previously CLAMPED to ±greatestFiniteMagnitude (and NaN to 0), which
        // silently disagreed with the T2 guest and the host: `.frame(maxWidth:
        // .infinity)` — a ubiquitous idiom — arrived as 1.797e308. SwiftUI's layout
        // special-cases `.infinity` but not a huge finite value, so the substitution
        // was a fidelity bug as well as a drift bug.
        if !d.isFinite {
...
public enum EmbeddedJSON {
    ...
    static func emitEmission(_ e: BodyEmission, into out: inout JSONOut) {
        out.object { o in
            ...
            // PARAMETERIZED NATIVE SLOTS — `[String:[String]]?` (slot id → lifted
            // string-literal values). Matches Swift's synthesized Codable for a
            // `[String:[String]]` dictionary: a JSON OBJECT keyed by the slot ids.
            // Emitted in SORTED key order so the bytes are deterministic (the host
            // `JSONDecoder` is order-insensitive; determinism keeps tests stable).
    ...
    static func emitKind(_ kind: NodeKind, into out: inout JSONOut) {
        ...
        case .callbackSlot(let id, let label):
            // IR schema v12: a child-view call whose `() -> Void` closure body lowers to a WASM
            // dispatch sequence. The node carries only the position-stable `id` and a diagnostic
            // `label`; the native slot closure (the full child-view rendering) rides the thunk's
            // `__patchCallbackSlots()` opaque table (same as a standard `.opaque` slot, distinct
            // from the callback dispatch mechanism). An OLD SDK sees "callbackSlot" as an unknown
            // discriminator key and throws on decode — the per-view v12 minVersion gate ensures
            // the older host DEMOTES this view to native instead of crashing.
