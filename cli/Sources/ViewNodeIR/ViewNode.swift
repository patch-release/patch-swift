...
public indirect enum NodeKind: Equatable, Sendable {
    ...
    /// A CHILD-VIEW CALL with a CLOSURE ARGUMENT whose body is a DISPATCHABLE action —
    /// e.g. `TeaserScreen(onContinue: { showPaywall = true })` or
    /// `PaywallView(onPurchase: { isPurchased = true; analytics.track("purchase") })`.
    /// The ENTIRE CALL (child view + all its other arguments) lowers as a MIXED SLOT,
    /// but the CLOSURE ARG's body lowers as a WASM DISPATCH SEQUENCE: the native thunk
    /// gets a STABLE forwarding closure `{ self.__patchDispatchCallback("<id>") }` which
    /// calls `patch_host.dispatch_callback`, the WASM body executes the real state-mutation
    /// logic, and the view re-renders. This makes the closure body OTA-patchable (editing
    /// `showPaywall = true` → `showPaywall = true; dismiss()` ships OTA with ZERO fingerprint
    /// churn). The position-keyed `id` (view-type + closure-arg label + occurrence-index) is
    /// STABLE across body edits so the thunk's `__patchCallbackSlots()` can supply the
    /// forwarding closure even after an edit.
    ///
    /// SOUNDNESS: the WHOLE SLOT is subject to the standard slot demote-safety net
    /// (uncovered slot id → demote). The `callbackSlot` node carries NO subtree; the
    /// native slot closure supplies the full view for the child-call position. What changes
    /// OTA is only the WASM body of the callback — not the child view structure itself.
    ///
    /// SCHEMA v12 (additive). An OLD SDK sees this as an unknown `NodeKind` case and fails
    /// to decode the whole view tree, per the standard `Codable` synthesis (throws on an
    /// unknown discriminator key) — per-view schema gate (minVersion 12) ensures an older
    /// host DEMOTES this view to native instead of crashing.
...
extension ViewNode {
    ...
    public var childNodes: [ViewNode] {
        ...
        case .text, .styledText, .dateText, .image, .symbolImage, .bundleImage,
            ...
            // `callbackSlot` also has NO tree children — the native slot closure
            // supplies the full child-view rendering; only the callback body is OTA.
            return []
        }
    }
}
